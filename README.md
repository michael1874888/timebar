# TimeBar v3.1 ⏱️💰

> 把每一筆消費轉換成「退休時間成本」，讓你更清楚每個財務決策對未來的影響。

## 🎉 v3.1 重大更新

- 🔋 **生命電池視覺化**：將人生以電池呈現，即時看到消費對自由時間的影響
- 🎯 **每日挑戰系統**：4 個低門檻任務，輕鬆養成省錢習慣
- 🏃 **追趕計劃**：落後目標時提供可行的追回方案
- 📊 **支出分類餅圖**：直觀顯示消費分佈和「生命殺手」
- ✅ **Toast 通知**：記錄成功時即時回饋
- 💪 **正向訊息框架**：所有提示都以積極行動導向呈現

## ✨ 功能特色

### 核心計算
- **時間成本轉換**：將每筆花費/儲蓄轉換為「延後/提早退休」的天數
- **複利計算**：考慮通膨率和投資報酬率，計算實質報酬率
- **週期性支出**：一次性 vs 每月固定支出，展示訂閱服務的真正代價

### GPS 導航系統
- **目標 vs 預估**：目標退休年齡固定，預估年齡根據行為動態計算
- **即時反饋**：每筆記錄都會更新「領先/落後」狀態
- **視覺化進度條**：直觀顯示與目標的差距

### 退休計算機（設定頁）
- **年齡導向**：設定退休年齡，計算可累積多少退休金
- **金額導向**：設定目標金額，計算需要工作到幾歲
- **生活品質導向**：設定退休後月領金額，計算所需存款（4%法則）

### 記錄追蹤
- 💸 **花費記錄**：追蹤消費並看到對退休的影響
- 💰 **儲蓄記錄**：記錄每筆儲蓄，看著退休日期提前
- 📊 **歷史統計**：按月分組查看，累積進度一目瞭然

### 資料儲存
- 📱 **本地儲存**：使用 localStorage，關閉瀏覽器也不會遺失
- ☁️ **雲端同步**：連接 Google Sheets，跨裝置自動同步

---

## 🧪 測試

採用**精簡測試策略**，專注於核心邏輯和集成測試：

```bash
# 執行 90 個測試（精簡後）
npm test

# 執行結果
# ✅ 90 通過 (2 個測試文件)
# ⚡ 測試時間: ~17 秒
```

**測試覆蓋：**
- ✅ **financeCalc.test.ts** (56 tests) - 核心財務計算邏輯
  - 複利計算、年金計算、4% 法則
  - 時間格式化、GPS 退休年齡預估
  - 邊界案例和數值精度

- ✅ **App.test.tsx** (34 tests) - 端到端集成測試
  - 完整用戶流程：Onboarding → Dashboard → 記帳 → 歷史 → 設定
  - 導航和狀態管理
  - Google Sheets 同步
  - 錯誤處理和邊緣情況

**測試哲學：**
> 不測試 UI 實現細節，只測試核心業務邏輯和用戶關鍵流程。
> 保持測試穩定，避免因 UI 調整而頻繁修改測試。

---

## 📁 專案結構

```
timebar/
├── src/
│   ├── components/              # React 元件 (TypeScript)
│   │   ├── App.tsx              # 主應用
│   │   ├── App.test.tsx         # 集成測試（34 個）
│   │   ├── Confetti.tsx         # 慶祝動畫
│   │   ├── dashboard/
│   │   │   ├── DashboardScreen.tsx
│   │   │   ├── LifeBattery.tsx
│   │   │   ├── DailyChallenge.tsx
│   │   │   └── CatchUpPlan.tsx
│   │   ├── onboarding/
│   │   │   └── OnboardingScreen.tsx
│   │   ├── tracker/
│   │   │   └── MainTracker.tsx
│   │   ├── history/
│   │   │   ├── HistoryPage.tsx
│   │   │   └── CategoryPieChart.tsx
│   │   └── settings/
│   │       └── SettingsPage.tsx
│   ├── services/
│   │   └── googleSheets.ts      # API 服務
│   ├── utils/
│   │   ├── financeCalc.ts       # 核心計算邏輯
│   │   ├── lifeCostCalc.ts      # 生動比喻計算
│   │   ├── storage.ts           # localStorage 封裝
│   │   └── helpers.ts           # UI 工具函數
│   ├── constants/
│   │   └── index.ts             # 環境變數
│   ├── types/
│   │   └── index.ts             # 類型定義
│   ├── tests/
│   │   ├── financeCalc.test.ts  # 核心邏輯測試（56 個）
│   │   └── setup.ts             # 測試環境設定
│   ├── styles/
│   │   ├── index.css            # Tailwind + 全域樣式
│   │   └── animations.css       # 動畫定義
│   ├── main.tsx                 # React 入口點
│   └── vite-env.d.ts            # Vite 環境類型
├── google-apps-script/
│   └── Code.gs                  # Google Apps Script 後端
├── index.html                   # Vite 開發入口
├── vite.config.js               # Vite 設定
├── tailwind.config.js           # Tailwind 設定
├── tsconfig.json                # TypeScript 配置
├── tsconfig.node.json           # TypeScript Node 配置
├── vitest.config.ts             # Vitest 測試設定
├── package.json                 # 專案設定
├── .env.local                   # 環境變數（不提交）
└── README.md
```

---

## 🚀 快速開始

### 方法一：本地開發（推薦）

**環境需求**：
- Node.js 18+ (建議 20 LTS)
- npm 9+

**安裝步驟**：

```bash
# 1. 安裝依賴
npm install

# 2. 設定環境變數（可選，用於 Google Sheets 同步）
cp .env.example .env.local
# 編輯 .env.local，填入你的 Google Apps Script URL

# 3. 啟動開發伺服器
npm run dev

# 4. 開啟瀏覽器
# http://localhost:5173/timebar/
```

**開發指令**：

```bash
npm run dev      # 啟動開發伺服器（含熱重載）
npm run build    # 打包生產版本
npm run preview  # 預覽打包結果
npm test         # 執行測試
```

### 方法二：GitHub Pages 部署

1. **Fork 這個專案**
2. **設定環境變數**
   - 在 Repo → Settings → Secrets → Actions
   - 新增 `VITE_GAS_URL`（你的 Google Apps Script URL）
3. **啟用 GitHub Pages**
   - Settings → Pages → Source 選 **GitHub Actions**
4. **Push 到 main** → 自動建置並部署
5. **完成！** 訪問 `https://你的帳號.github.io/timebar/`

### 方法三：手動部署

```bash
npm run build
# 將 dist/ 資料夾內容部署到任何靜態網站服務
```

---

## ☁️ 設定 Google Sheets 雲端同步

### 步驟 1：建立 Google Sheets

1. 開啟 [Google Sheets](https://sheets.google.com)
2. 建立新的空白試算表
3. **複製試算表 ID**（從網址中取得）：
   ```
   https://docs.google.com/spreadsheets/d/【這一段就是ID】/edit
   ```
   例如：`1bJ-plKk3locg4fRWtEcdL5rIgT2KQvEz5Xer1h7--MU`

### 步驟 2：建立 Apps Script

1. 在試算表中，點選 **擴充功能** → **Apps Script**
2. 刪除預設的程式碼
3. 複製 `google-apps-script/Code.gs` 的內容並貼上
4. **重要！** 在第 15 行填入你的試算表 ID：
   ```javascript
   const SPREADSHEET_ID = '貼上你的試算表ID';
   ```
5. 點擊 **儲存** (Ctrl+S)

### 步驟 3：測試連接

1. 在函數下拉選單選擇 `testConnection`
2. 點擊 ▶️ 執行
3. 首次會要求授權，點「授權存取」→「繼續」→ 選擇帳號 →「允許」
4. 查看執行紀錄，應顯示 `✅ 連接成功！`

### 步驟 4：部署為網頁應用程式

1. 點選 **部署** → **新增部署作業**
2. 點擊齒輪圖示，選擇 **網頁應用程式**
3. 設定：
   - 執行身分：**我**（不要改）
   - 誰可以存取：**所有人**
4. 點擊 **部署**
5. **複製產生的網址**

### 步驟 5：連接到 TimeBar

**本地開發環境**：
1. 編輯 `.env.local` 檔案
2. 設定你的 URL：
   ```bash
   VITE_GAS_URL=https://script.google.com/macros/s/xxx.../exec
   ```
3. 重新啟動開發伺服器：`npm run dev`

**GitHub Pages 部署**：
1. Repo → Settings → Secrets → Actions
2. 新增 Secret：`VITE_GAS_URL`
3. 值填入你的 Google Apps Script URL
4. Push 到 main 分支，自動建置並部署

### 驗證成功

- App 設定頁面會顯示「Google Sheets 已連接」
- 記錄消費/儲蓄後，Google Sheets 會自動新增資料
- 「消費紀錄」分頁：綠色=儲蓄、紅色=花費
- 「使用者資料」分頁：包含每月儲蓄、目標退休金、通膨率、投資報酬率

> ⚠️ **升級注意**：從 v2.1 升級需刪除 Google Sheets 中的「使用者資料」分頁，讓系統重新建立新格式。

---

## 🧮 計算邏輯

### 公式

**一次性消費/儲蓄：**
```
未來價值 = 金額 × (1 + 3.41%)^距離退休年數
時間成本 = 未來價值 ÷ 時薪
```

**每月固定消費/儲蓄：**
```
未來價值 = 月付金額 × ((1 + 月報酬率)^月數 - 1) ÷ 月報酬率
時間成本 = 未來價值 ÷ 時薪
```

### 預設參數

| 參數 | 預設值 | 可調整 | 說明 |
|------|--------|--------|------|
| 通膨率 | 2.5% | ✅ 0-10% | 長期平均 |
| 投資報酬率 | 6% | ✅ 0-15% | 大盤長期平均 |
| 實質報酬率 | ≈3.41% | 自動計算 | (1+ROI)/(1+通膨)-1 |
| 每月工作天數 | 22 天 | ❌ | - |
| 每日工時 | 8 小時 | ❌ | - |

---

## ❓ 常見問題

### Q: Google Sheets 沒有更新？
1. 確認 `SPREADSHEET_ID` 是否正確填入
2. 確認部署後有選擇「新版本」
3. 執行 `testConnection` 測試連接

### Q: 每次記錄會觸發兩個請求？
正常！一個儲存使用者資料，一個儲存消費紀錄。

### Q: 如何更新 Apps Script？
修改程式碼後：**部署** → **管理部署作業** → ✏️ **編輯** → 版本選「新版本」→ **部署**

### Q: 清除資料後頁面沒反應？
v2.1 已修正此問題，會自動跳轉到初始設定頁面。

### Q: 可以調整通膨率和報酬率嗎？
可以！在設定頁面的「計算參數」區塊可以自訂。

---

## 📄 License

MIT License

---

> 「每一分錢都是你未來自由的磚塊」🧱

Made with ❤️ for financial freedom
